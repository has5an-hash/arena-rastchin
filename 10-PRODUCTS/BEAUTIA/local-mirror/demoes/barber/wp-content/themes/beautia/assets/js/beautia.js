...
(function () {
  ...
  (function () {
    ...
    if(root.dataset.beautiaAppearance === 'pastel'){root.removeAttribute('data-theme');return;}
  ...
  if (sToggle && sBox) {
    ...
    if(sInput&&sResults) sInput.addEventListener('input',function(){
      ...
      if(q.length<2){sResults.innerHTML='<p>حداقل دو حرف بنویسید؛ مثلاً «کوتاهی» یا «فیشیال».</p>';return;}
      searchTimer=setTimeout(function(){
        ...
        fetch(url,{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(res){
          ...
          if(!items.length){sResults.innerHTML='<p>نتیجه‌ای پیدا نشد؛ عبارت کوتاه‌تری امتحان کنید.</p>';return;}
          sResults.innerHTML=items.map(function(item){var media=item.image?'<img src="'+item.image+'" alt="">':'<span class="search-suggestion-placeholder">✦</span>';return '<a class="search-suggestion" href="'+item.url+'">'+media+'<strong>'+item.title+'</strong><small>'+item.type+'</small></a>';}).join('');
        }).catch(function(){sResults.innerHTML='<p>جستجو در دسترس نیست؛ دوباره تلاش کنید.</p>';});
      },220);
    });
  }
